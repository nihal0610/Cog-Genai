...
#                 with st.sidebar:
#                     st.title("Please select in the drop down if u want to find the status of the project")
...
# Read the CSV file
ddf = pd.read_csv(r'C:\Users\2216184\OneDrive - Cognizant\Desktop\utilisation\UTIL_ALL.csv')
...
input1 = "Give me IDs of all managers in a list format, whose days_passed is greater than 60 and project type is EXANT "
...
def get_result():
    result1 = agent.invoke(input1)
    value = result1['output']
    return value
 
# Create Gradio interface
iface = gr.Interface(fn=get_result, inputs=None, outputs="text")
...
#             button = st.button("Submit")
#         input1=f"Give me IDs of managers, Project name in a python dataframe with each item should be iterable, whose Days {e} from {b} is {d} {c} and project type is {a}"
#         if button is True:  
